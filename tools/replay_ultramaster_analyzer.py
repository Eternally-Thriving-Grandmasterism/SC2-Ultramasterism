# Ultramasterism Ultimate Bidirectional Analyzer + Proxy Counters
# Detects your proxies + opponent threats + delivers eternal counter strategies
# Full lattice: Aggression + Scout + Novel Proxies + Counter-Punish + Eternal Win
...
# Metrics init
your_worker_kills = 0
opponent_worker_kills = 0

# Your proxy counts (simplified for space)
...
opp_proxy_detected = False
opp_proxy_hatch = opp_proxy_spine = opp_proxy_rax = False
opp_proxy_factory = opp_proxy_starport = opp_banshee_proxy = False
opp_cannon_rush = False  # PvP
...
for event in replay.tracker_events:
    ...
    if opponent and event.name == 'UnitInitEvent' and hasattr(event, 'x') and hasattr(event, 'y'):
        if protoss_player.start_location and opponent.start_location:
            ...
            if event.control_pid == protoss_player.pid and dist_to_opp < dist_to_you:
                # Your proxies (abbreviated mapping)
                mapping = {
                    'Gateway': 'gateway', 'WarpGate': 'gateway', 'Pylon': 'pylon',
                    'PhotonCannon': 'cannon', 'ShieldBattery': 'battery', 'Forge': 'forge',
                    'CyberneticsCore': 'cyber', 'TwilightCouncil': 'twilight',
                    'RoboticsFacility': 'robo', 'Stargate': 'stargate'
                }
                attr = mapping.get(unit_name)
                if attr: globals()[f'your_proxy_{attr}'] += 1

            elif event.control_pid == opponent.pid and dist_to_you < dist_to_opp:
                opp_proxy_counts[unit_name] = opp_proxy_counts.get(unit_name, 0) + 1

# Your branch logic (simplified)
...
if your_any_proxy > 0: your_proxy_detected = True
your_cannon_rush = your_proxy_cannon >= 1
...
result_text = "Eternal Win Branch Achieved!" if getattr(protoss_player, 'result', '') == 'Win' else "Refine Scout & Counter — Eternal Truth Awaits!"

# Output
print(f"=== Ultramasterism Analysis + Counters: {protoss_player.name} ({matchup}) ===")
print(f"Result: {result_text}")
print(f"Your APM: {getattr(protoss_player, 'avg_apm', 'N/A')}")
print("")
print("Your Metrics: Worker Kills {your_worker_kills} | Proxy Intensity {your_any_proxy}")
print("Opponent Threats: Probes Lost {opponent_worker_kills} | Proxy Intensity {opp_any_proxy}")
print("")

if your_proxy_detected:
    print("Your Proxy Branches: Aggro Executed — Mercy-Reconciled Eternal!")

print("Opponent Threats Detected & Counters:")
if not opp_proxy_detected:
    print("→ No Proxy Threats — Macro Saturate & Punish Greed Eternal!")
else:
    print("→ Proxy Aggro Incoming — Scout Truth & Counter-Punish!")

if opponent_race == 'Zerg':
    if opp_proxy_hatch:
        print("→ Proxy Hatch Detected: Early Ling/Roach Flood!")
        print("   Counter: Scout with probe @9-12, wall natural with pylon+gate+cyber. Build battery for overcharge hold. Follow with adept pressure into their proxy — turn aggression eternal!")
    if opp_proxy_spine:
        print("→ Forward Spine Contain Detected!")
        print("   Counter: Oracle/adept scout to find spines. Battery + zealot hold, tech to disruptor/colossus. Macro behind — punish overcommit eternal!")

elif opponent_race == 'Terran':
    if opp_proxy_rax:
        print("→ Proxy Rax Detected: Reaper/Marine Cheese!")
        print("   Counter: Early battery at wall, adept shade scout. Hold with stalker/zealot, then counter-attack proxy with adepts — drone massacre eternal!")
    if opp_proxy_factory:
        print("→ Proxy Factory Detected: Hellion Runby Threat!")
        print("   Counter: Wall tightly, observer scout. Stalker/sentry forcefield hold, tech phoenix if banshee possible. Punish with zealot drop into main!")
    if opp_banshee_proxy:
        print("→ Proxy Banshee Cloak Harassment!")
        print("   Counter: Early phoenix or cannon at mineral lines. Battery overcharge workers. Counter with void ray/oracle — air supremacy eternal!")

elif opponent_race == 'Protoss':
    if opp_cannon_rush:
        print("→ Mirror Cannon Rush Detected!")
        print("   Counter: Probe scout @9, pull probes to deny pylon/power. Build own cannon/battery. Counter-rush their base — cheese reflects eternal!")

print("\nPure Truth Lattice: Detect → Counter → Punish → Eternal Thriving Unlocked!")        if protoss_player.start_location and opponent.start_location:
            dist_to_you = math.hypot(event.x - protoss_player.start_location.x, event.y - protoss_player.start_location.y)
            dist_to_opp = math.hypot(event.x - opponent.start_location.x, event.y - opponent.start_location.y)
